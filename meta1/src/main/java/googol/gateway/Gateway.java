...
public class Gateway extends UnicastRemoteObject implements GatewayService {
    ...
    public void registerClient(ClientCallback client) throws RemoteException {
        if (!registeredClients.contains(client)) {
            ...
            // Send initial stats
            try {
                // If the client prefers a single-string update, send that, otherwise send the three updates
                if (client instanceof SimpleClientCallback) {
                    String stats = buildPlainStatsString(currentTop10, getBarrelStatus(), getAverageResponseTimes());
                    ((SimpleClientCallback) client).updateStatsString(stats);
                } else {
    ...
    private void updateTop10IfChanged() {
        ...
        if (!oldStr.equals(newStr)) {
            ...
            // Build a single plain-text stats string to send to simple clients
            String statsString = buildPlainStatsString(newTop10, barrelStatus, responseTimes);

            notifyClients(client -> {
                if (client instanceof SimpleClientCallback) {
                    ((SimpleClientCallback) client).updateStatsString(statsString);
                } else {
    ...
    private String buildPlainStatsString(String[][] top10, Map<String, Integer> barrelStatus, Map<String, Double> responseTimes) {
        StringBuilder sb = new StringBuilder();
        sb.append("TOP10:");
        for (int i = 0; i < top10.length; i++) {
            String term = top10[i][0];
            String count = top10[i].length > 1 ? top10[i][1] : "0";
            sb.append(term).append("(").append(count).append(")");
            if (i < top10.length - 1) sb.append(",");
        }
        sb.append(";BARREL STATUS:");
        int bi = 0;
        for (Map.Entry<String, Integer> e : barrelStatus.entrySet()) {
            sb.append(e.getKey()).append("=").append(e.getValue());
            if (++bi < barrelStatus.size()) sb.append(",");
        }
        sb.append(";AVG RESPONSE TIMES:");
        int ri = 0;
        for (Map.Entry<String, Double> e : responseTimes.entrySet()) {
            sb.append(e.getKey()).append("=").append(e.getValue());
            if (++ri < responseTimes.size()) sb.append(",");
        }
        return sb.toString();
    }
